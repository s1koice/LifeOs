...
`vercel.json` запускает проверку каждый час (`0 * * * *`) и сравнивает текущий
час в вашем часовом поясе с временем чекина из Настроек — так утренний и
вечерний разбор подстраиваются под конфигурацию без правки `vercel.json`.
